...
vector<Absenta> Elev::getAbsente() {
    return this->absente;
}

void Elev::stergereAbsenta(Absenta absenta) {
    for (auto it = this->absente.begin();it!=absente.end();it++)
    {
        if (it->getData()==absenta.getData()) {
            this->absente.erase(it);
            break;
        }
